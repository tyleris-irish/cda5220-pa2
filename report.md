...
**N = 200,000**, **P = 20 ms** (K = 250 slots over 5 s).

N is chosen so the array (~25 MB at 128-byte stride) exceeds the LLC, making cache evictions from a victim page detectable. P = 20 ms balances noise while getting enough sweeps — larger P loses the shape of the page load; smaller P squeezes too few sweeps into each slot and collapses the signal.
